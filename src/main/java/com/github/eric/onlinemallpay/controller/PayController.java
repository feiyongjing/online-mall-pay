...
public class PayController {
    ...
    public ModelAndView create(@RequestParam("orderId") String orderId,
        ...
        // 下单，订单信息写入数据库
        PayInfo payInfo = buildPayInfo(orderId, amount, bestPayTypeEnum);
    ...
    private PayInfo buildPayInfo(@RequestParam("orderId") String orderId, @RequestParam("amount") BigDecimal amount, @RequestParam("payType") BestPayTypeEnum bestPayTypeEnum) {
        PayInfo payInfo = new PayInfo();
        payInfo.setOrderNo(Long.valueOf(orderId));
        payInfo.setPayPlatform(PayPlatformEnum.getByPayPlatformEnum(bestPayTypeEnum).getCode());
    ...
    public Object asyncNotify(@RequestBody String notifyData) {
        ...
        // 判断数据库的数据是否是未支付
        if (!Objects.equals(payInfo.getPlatformStatus(), OrderStatusEnum.SUCCESS.getDesc())) {
            // 未支付
            ...
            rabbitTemplate.convertAndSend(RabbitmqConfig.TEST_EXCHANGE_HAHA,RabbitmqConfig.TEST_ROUTING_KEY_HAHA, payInfo);
      }
